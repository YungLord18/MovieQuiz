...
class MoviesLoaderTests: XCTestCase {
    func testSuccessLoading() throws {
        ...
        //When
        //Так как функция загрузки фильмов ассинхронное нужно добавить ожидание
        ...
        loader.loadMovies { result in
            ...
            case .success(let movies):
                //проверим что пришло например 2 фильма
                XCTAssertEqual(movies.items.count, 2)
                expectation.fulfill()
            case .failure(_):
                //мы не ожидаем что придет ошибка, если она появится надо провалить тест
